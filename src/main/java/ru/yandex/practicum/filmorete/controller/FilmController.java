...
public class FilmController {
    ...
    public List<Film> getPopularFilms(
        ...
        log.info(
                "   GET [http://localhost:8080/films/popular?count={}&genreId={}&year={}] : " +
                        "Запрос списка популярных фильмов с возможностью фильтрации по году и жанру",
                count, genreId, year
        );
    ...
    public List<Film> getCommonFilms(@RequestParam Long userId, @RequestParam Long friendId) {
        log.info(
                "   GET [http://localhost:8080/films/common?userId={}&friendId={}] : " +
                        "Запрос получения общих фильмов друзей, отсортированные по популярности",
                userId, friendId
        );
    ...
    public List<Film> getFilmsBySearchParam(
        ...
        log.info(
                "   GET [http://localhost:8080/search?query={}&by={}] : " +
                        "Запрос получения фильмов по режиссеру и/или названию",
                query, by
        );
    ...
    public List<User> getUsersToLikeFilm(@PathVariable Long filmId) {
        log.info(
                "   GET [http://localhost:8080/{}/to-like] : " +
                        "Запрос списка пользователей которые поставили лайк фильму",
                filmId
        );
    ...
    public void addLikeFilm(@PathVariable Long filmId, @PathVariable Long userId) {
        log.info(
                "   PUT [http://localhost:8080/{}/like/{}] : " +
                        "Запрос пользователь {} ставит лайк фильму по id = {}",
                filmId, userId, userId, filmId);
    ...
    public void removeLikeFilm(@PathVariable Long filmId, @PathVariable Long userId) {
        log.info(
                "DELETE [http://localhost:8080/{}/like/{}] : " +
                        "Запрос пользователь удаляет лайк фильму по id",
                filmId, userId
        );
    ...
    public List<Film> getFilmsByDirectorSortedByParam(
        ...
        log.info(
                "   GET [http://localhost:8080/director/{}?sortBy={}] : " +
                        "Запрос на получение списка фильмов режиссера, отсортированных по количеству лайков или году выпуска",
                directorId,sortBy
        );
